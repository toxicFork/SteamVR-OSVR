...
OSVRHandDevice::OSVRHandDevice(osvr::clientkit::ClientContext& context, vr::IServerDriverHost* driver_host, vr::IDriverLog* driver_log) :
    OSVRTrackedDevice(context, driver_host, vr::TrackedDeviceClass_Controller, driver_log)
{
}

const char* OSVRHandDevice::GetId()
{
    /// @todo When available, return the actual unique ID of the HMD
    return "OSVR Hand";
}

vr::TrackedDeviceDriverInfo_t OSVRHandDevice::GetTrackedDeviceDriverInfo()
{
...
const char* OSVRHandDevice::GetModelNumber()
{
    /// @todo When available, return the actual model number of the HMD
    return "OSVR Hand";
}

const char* OSVRHandDevice::GetSerialNumber()
{
    /// @todo When available, return the actual serial number of the HMD
    return "0";
}

vr::EVRInitError OSVRHandDevice::Activate(uint32_t object_id)
{
    ...
    // register tracker callback
    m_TrackerInterface = m_Context.getInterface("/me/hands/left");
    m_TrackerInterface.registerCallback(&OSVRHandDevice::HandTrackerCallback, this);

    return vr::VRInitError_None;
}

void OSVRHandDevice::Deactivate()
{
    OSVRTrackedDevice::Deactivate();
}

vr::VRControllerState_t OSVRHandDevice::GetControllerState()
{
    // TODO
    vr::VRControllerState_t controller_state = vr::VRControllerState_t();

    controller_state.ulButtonPressed = false;
    controller_state.ulButtonTouched = false;
//#if 0
...
void OSVRHandDevice::HandTrackerCallback(void* userdata, const OSVR_TimeValue* timestamp, const OSVR_PoseReport* report)
{
    if (!userdata)
        return;

    auto* self = static_cast<OSVRHandDevice*>(userdata);
...
float OSVRHandDevice::GetFloatProperty(vr::ETrackedDeviceProperty prop, vr::ETrackedPropertyError* error)
{
